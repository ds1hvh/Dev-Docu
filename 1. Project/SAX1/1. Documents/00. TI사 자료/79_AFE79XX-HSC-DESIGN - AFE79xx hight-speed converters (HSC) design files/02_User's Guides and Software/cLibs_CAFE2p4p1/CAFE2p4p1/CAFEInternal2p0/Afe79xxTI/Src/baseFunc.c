...
uint8_t waitMs(uint32_t wait_ms)
{
    ...
    pause = wait_ms * (CLOCKS_PER_SEC / 1000);
    now = then = clock();
    while ((now - then) < pause)
    {
        now = clock();
    }
    afeLogSpiLog("delay of %d", wait_ms);
    return RET_OK;
}
...
void openLogFile(char *fileName, char *openMode)
{
    if (fileName == NULL)
    {
        fileName = "Afe79xx_Clog.txt";
    }
    logfp = fopen(fileName, openMode);
}
...
void afeLogmsg(uint32_t level, const char *pcLogFmt, ...)
{
    ...
    char output[1024];
    va_list args;
    va_start(args, pcLogFmt);
    vsprintf(output, pcLogFmt, args);
